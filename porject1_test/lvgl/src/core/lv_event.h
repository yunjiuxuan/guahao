...
#include <stdbool.h>
    ...
    typedef enum
    {
        ...
        LV_EVENT_PRESSING,            /**< 对象正在被按住（按下期间持续触发）*/
        LV_EVENT_PRESS_LOST,          /**< 对象仍被按住，但手指/光标已滑出对象区域 */
        LV_EVENT_SHORT_CLICKED,       /**< 对象被短按后释放（未发生滚动时触发）*/
        LV_EVENT_LONG_PRESSED,        /**< 对象被按住至少达到“长按时间”（未滚动时触发）*/
        LV_EVENT_LONG_PRESSED_REPEAT, /**< 长按后，每隔“重复间隔”持续触发（未滚动时触发）*/
        LV_EVENT_CLICKED,             /**< 释放时触发（只要未滚动，无论是否长按都会触发）*/
        LV_EVENT_RELEASED,            /**< 所有情况下，对象被释放时都会触发 */
        LV_EVENT_SCROLL_BEGIN,        /**< 开始滚动。事件参数是指向滚动动画的指针，可修改 */
        LV_EVENT_SCROLL_END,          /**< 滚动结束 */
        LV_EVENT_SCROLL,              /**< 正在滚动中 */
        LV_EVENT_GESTURE,             /**< 检测到手势。可通过 `lv_indev_get_gesture_dir(lv_indev_get_act())` 获取手势方向 */
        LV_EVENT_KEY,                 /**< 向对象发送了一个按键。可通过 `lv_indev_get_key(lv_indev_get_act())` 获取键值 */
        LV_EVENT_FOCUSED,             /**< 对象获得焦点 */
        LV_EVENT_DEFOCUSED,           /**< 对象失去焦点 */
        LV_EVENT_LEAVE,               /**< 对象失去焦点但仍处于选中状态（例如通过 Tab 切换焦点）*/
        LV_EVENT_HIT_TEST,            /**< 执行高级点击检测（用于自定义命中区域）*/
